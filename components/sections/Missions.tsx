      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
          <KineticText
            ...
            text="Missions"
            onScroll
            className="font-display text-5xl md:text-7xl text-[var(--text)]"
          />
          <p className="font-hud text-xs uppercase tracking-[0.28em] text-[var(--muted)]">
            {String(featured.length).padStart(2, "0")} campaign ops
          </p>
        </div>
      </div>
        ...
        {featured.map((p, i) => {
          ...
          return (
            ...
            >
              ...
              <span
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-r from-[color-mix(in_srgb,var(--accent)_8%,transparent)] to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"
              />
              ...
              <div className="flex flex-col gap-6 py-8 pl-5 pr-1 transition-transform duration-300 group-hover:translate-x-1 md:flex-row md:items-start md:gap-8">
                ...
                <div className="min-w-0 flex-1">
                  {/* classification chips: role + outcome medal */}
                  {(p.role || p.outcome) && (
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      ...
                      {p.outcome && (
                        <span
                          className={cn(
                            "inline-flex items-center gap-2 rounded-sm border border-[color-mix(in_srgb,var(--accent)_45%,transparent)] px-2.5 py-1",
